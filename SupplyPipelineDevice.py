...
class Medicamentos:
    ...
    def calcular(self, month):
        ...
        #     if type(indexes[i]) == type(datetime(2022, 6, 1, 0, 0)):
        #         indexes[i] = indexes[i].strftime("%b %Y").upper()
        #         # tmpIndexes.append(tmpC)
        #         # indexes[i]
        #     # else:
        #     #     tmpIndexes.append(c)
        # print(indexes)
        ...
        limit = 0
        for i in range(len(indexes)):        
            if indexes[i] == datetime.strptime(month.lower(), "%b %Y"):
                beginning = i
        ...
        for k in df:
            try:
                ...
                print('HERE',df[k][['AGV Forecast', 'DIREMADI Forecast', 'Forecast']]) #debug
            except:
                ...
