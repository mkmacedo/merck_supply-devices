...
class Medicamentos:
    ...
    def calcular(self, month):
        ...
        print('JDA_Cols',JDA_Cols)

        beginning = 0
        limit = 0
        for i in range(len(indexes)):        
            if type(indexes[i]) == type(datetime(2022, 6, 1, 0, 0)) and indexes[i].strftime('%b %Y').upper() == month:
                beginning = i
                limit = min(len(indexes), len(JDA_Cols))


        for i in range(0, len(self.df_forecast), 2):
        ...
        for i in range(1, len(self.df_forecast), 2):
            ...
            if code in codes:
                ...
                if self.df_forecast.loc[i, 'Key Figure'] == 'AGV Forecast':
                    ...
                    df[code]['DIREMADI Forecast'] = forecast[list(df[code]['Meses'])].values
                    # print('DIREMADI', code, self.df_forecast.loc[i, 'Key Figure']) #debug
                    # print(df[code]['DIREMADI Forecast'])
